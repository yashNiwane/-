...
class _ProfileCreationScreenState extends State<ProfileCreationScreen> {
        ...
        setState(() {
          ...
          if (data['height'] != null) {
            ...
            if (parts.isNotEmpty) {
              ...
              if (parts.length > 1) {
                _heightInchesController.text = parts[1].replaceAll('"', '').trim();
              }
            }
          }
  ...
  Future<void> _submitProfile() async {
    ...
    try {
      ...
      final biodataUrl = await _uploadFile(_biodataFile!, 'documents', 'biodata_$userId');

      if (profileUrl == null || biodataUrl == null) throw Exception('File upload failed');
      ...
      await _supabase.from('profiles').upsert({
        ...
        'height': "${_heightFeetController.text.trim()}' ${_heightInchesController.text.trim()}\"",
  ...
  Widget _buildSelectionOption(String label, IconData icon, String selectedValue, Function(String) onSelect) {
  ...
  */
  ...
  Widget _buildSimpleStep({required String title, required String subtitle, required Widget child}) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    return Padding(
